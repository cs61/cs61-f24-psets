...
sub read_expected ($) {
    ...
    my($allow_asan_warning, $time, $abort) = (0, 0, 0);
    ...
    return {"l" => \@expected, "nl" => scalar(@expected),
            ...
            "allow_asan_warning" => $allow_asan_warning, "abort" => $abort};
}
...
sub run_compare ($$$$$$) {
    ...
    for (; $a != @$actual; ++$a) {
        ...
        if (!@explines) {
            if ($exp->{"abort"} && /^alloc (?:count|size)/) {
                print STDERR "$Red${outname}FAIL: Statistics printed (expected test to abort)$Off\n";
    ...
    if ($exp->{"abort"} && $out->{"status"} == 0) {
        print STDERR "$Red${outname}FAIL: Exited with status $out->{status} (expected test to abort)$Off\n";
        return 1;
    }
