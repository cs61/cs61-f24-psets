...
// io61_read_bytes(f, buf, sz)
//    Read a block of `sz` bytes into `buf`, but using `io61_readc` calls.

ssize_t io61_read_bytes(io61_file* f, unsigned char* buf, size_t sz) {
    size_t nr = 0;
    while (nr != sz) {
        int ch = io61_readc(f);
        if (ch < 0) {
...
// io61_write_bytes(f, buf, sz)
//    Write a block of `sz` bytes from `buf`, but using `io61_writec` calls.

ssize_t io61_write_bytes(io61_file* f, const unsigned char* buf, size_t sz) {
    size_t nw = 0;
    while (nw != sz) {
...
void io61_args::usage() {
    ...
    if (strchr(this->opts, 'R')) {
        fprintf(stderr, "    -R            Read bytes, not blocks\n");
    }
    if (strchr(this->opts, 'W')) {
        fprintf(stderr, "    -W            Write bytes, not blocks\n");
    }
